...
ActiveSupport.on_load(:active_record) do
  class ActiveRecord::Base
    def self.hcafe2_define_common_relationships(key)
      has_many :metas, -> { where(object_class: key) }, class_name: 'Meta', foreign_key: :objectid, dependent: :destroy
      #has_many :custom_field_values, ->{ where(object_class: key) }, :class_name => "hcafe2leonCms::CustomFieldsRelationship", foreign_key: :objectid, dependent: :delete_all
      #has_many :custom_fields, ->{ where(:object_class => key)} , :class_name => "hcafe2leonCms::CustomField" ,foreign_key: :objectid

      # valid only for simple groups and not for complex like: posts, post, ... where the group is for individual or children groups
      #has_many :custom_field_groups, ->{ where(object_class: key) }, :class_name => "hcafe2leonCms::CustomFieldGroup", foreign_key: :objectid
    end
    
    # save cache value for this key
    def hcafe2_set_cache(key, val)
      @hcafe2_cache_vars ||= {}
      @hcafe2_cache_vars[hcafe2_build_cache_key(key)] = val
      val
    end

    # remove cache value for this key
    def hcafe2_remove_cache(key)
      @hcafe2_cache_vars.delete(hcafe2_build_cache_key(key))
    end

    # fetch the cache value for this key
    def hcafe2_fetch_cache(key)
      @hcafe2_cache_vars ||= {}
      _key = hcafe2_build_cache_key(key)
      if @hcafe2_cache_vars.has_key?(_key)
        # puts "*********** using model cache var: #{_key}"
        @hcafe2_cache_vars[_key]
      else
        @hcafe2_cache_vars[_key] = yield
        @hcafe2_cache_vars[_key]
      end
    end

    # return the cache value for this key
    def hcafe2_get_cache(key)
      @hcafe2_cache_vars ||= {}
      @hcafe2_cache_vars[hcafe2_build_cache_key(key)] rescue nil
    end

    # internal helper to generate cache key
    def hcafe2_build_cache_key(key)
      _key = "hcafe2_cache_#{self.class.name}_#{self.id}_#{key}"
    end

    # check if an attribute was changed
    def hcafe2_attr_changed?(attr_name)
