class Admin::AdminController < ApplicationController
  ...
  before_action :load_initial_data

  before_action :authenticate_user!, except: []
  before_action :check_is_admin, except: []
  
  # cancancan으로 posts의 authorization을 처리하겠다는 의미
  load_and_authorize_resource
  
  around_action :run_using_master
  ...
  def check_is_admin
    redirect_to root_path if !has_admin_permission? 
  end 
  
  def has_admin_permission? 
    current_user.admin_group? || current_user.moderator_group?
  end 
