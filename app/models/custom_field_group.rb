class ::CustomFieldGroup < ::CustomField
  ...
  default_scope { where.not(object_class: '_fields')
    .reorder("#{::CustomField.table_name}.field_order ASC") }

  has_many :metas, -> { where(object_class: 'CustomFieldGroup') }, class_name: '::Meta',
    foreign_key: :objectid, dependent: :destroy
  has_many :fields, -> { where(object_class: '_fields') }, class_name: '::CustomField',
  ...
  def auto_save_default_values(field, options)
    ...
    if %w(Post Category Plugin Theme).include?(class_name) && objectid && (options[:default_value].present? || options[:default_values].present?)
      if class_name == 'Theme'
        owner = "::#{class_name}".constantize.find(objectid) # owner model
      else
        owner = "::#{class_name}".constantize.find(objectid) rescue "::#{class_name}".constantize.find_by(slug: objectid) # owner model
      end
