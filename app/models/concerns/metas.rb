module Metas extend ActiveSupport::Concern
  included do
    ...
    has_many :metas, ->(object){ where(object_class: object.class.to_s.gsub("Decorator","")) }, class_name: "Meta", foreign_key: :objectid, dependent: :delete_all
  end
  ...
  def set_meta(key, value)
    ...
    hcafe2_set_cache("meta_#{key}", value)
  end
  ...
  def get_meta(key, default = nil)
    ...
    hcafe2_fetch_cache("meta_#{key_str}") do
  ...
  alias_method :hcafe2_options, :options
  ...
  def get_option(key = nil, default = nil, meta_key = "_default")
    values = hcafe2_options(meta_key)
    key = key.to_sym
  ...
  def set_options(h = {}, meta_key = "_default")
    if h.present?
      data = hcafe2_options(meta_key)
      fixActionParameter(h).each do |key, value|
        data[key] = fix_meta_var(value)
      end
      set_meta(meta_key, data)
    end
