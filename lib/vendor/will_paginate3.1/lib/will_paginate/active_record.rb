  ...
  module ActiveRecord
    ...
    module Pagination
      ...
      def paginate(options)
        paging_table_provider = options[:paging_table_provider]
        options  = options.dup
        pagenum  = options.fetch(:page) { raise ArgumentError, ":page parameter required" }
        options.delete(:page)
        per_page = options.delete(:per_page) || self.per_page
        total    = options.delete(:total_entries)
        use_paging_table = options.delete(:use_paging_table) || false 
        paging_key = options.delete(:paging_key) || primary_key
        options.delete(:paging_table_provider)
        ...
        if use_paging_table && paging_table_provider.nil?
          raise ArgumentError, "use_paging_table: true, but paging_table_provider not provided"
        end 
        
        if !use_paging_table || paging_table_provider.nil? then # by etna 
          rel = limit(per_page.to_i).page(pagenum)
          rel.total_entries = total.to_i unless total.blank?
          rel
        else 
          curpage_gid = paging_table_provider.id_for_page(pagenum)
          rel = range(curpage_gid, per_page, pagenum, paging_key)
          rel.total_entries = total.to_i unless total.blank?
          rel 
        end 
      end

      def page(num, use_paging_table=false)
      ...
      def range(index_start, per_page, pagenum, paging_key) 
        rel = if ::ActiveRecord::Relation === self
          self
        elsif !defined?(::ActiveRecord::Scoping) or ::ActiveRecord::Scoping::ClassMethods.method_defined? :with_scope
          # Active Record 3
        ...
        rel = rel.extending(RelationMethods)
        pagenum1 = ::WillPaginate::PageNumber(pagenum)
        #per_page = rel.limit_value || self.per_page
        rel = rel.where("#{paging_key} >= ?", index_start)
        #rel = rel.where("#{primary_key} < ?", range2)
        #rel = rel.offset(pagenum.to_offset(per_page).to_i)
        rel = rel.limit(per_page) unless rel.limit_value
        rel.current_page = pagenum1
        #rel.per_page = 20
        rel
      end 
